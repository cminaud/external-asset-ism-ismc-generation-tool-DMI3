...
class Audio(BaseModel):
    ...
    @classmethod
    def from_xml(cls, audio_element: ET.Element, namespace: dict):
        attributes = audio_element.attrib
        src = attributes.get('src')
        system_bitrate = attributes.get('systemBitrate')
        system_language = attributes.get('systemLanguage')
        params = audio_element.findall(f'{list(namespace.keys())[0]}:param', namespace)
        param_list = []
        for param in params:
